...
2. **Broker replay / capture (now highest value)** — `broker send <command> [payload]` is now implemented in `psn-direct-cli.ts`, a local `broker:emulator` shim now exists for app-free broker-client iteration on `ws://localhost:1235/`, and a macOS `broker:shell-harness` can now run Sony's real public bundle against that local broker to capture both the BrowserAPI sequence and the require-able PCClientAPI sequence. The BrowserAPI harness confirms `testConnection -> setSettings -> requestClientId -> setTitleInfo -> setAuthCodes -> requestGame`; the Mac PCClientAPI harness now additionally confirms PC-side `WINDOWS` / `PC` settings, `requestClientId`, `getStreamServerAuthCode()`, auth-code staging folded back into later `setSettings`, `requestGame`, escalation to `startGame` once a synthetic `GotLaunchSpec` is emitted, and post-launch routing commands `routeInputToClient` / `routeInputToPlayer`. The next task is to keep using the mock/harness for outbound-envelope iteration while using a live running app for capturing real replies to the PC-native `requestClientId`, `setSettings`, `requestGame`, `startGame`, and post-launch event/input path. Known commands: `startGame`, `stop`, `requestGame`, `requestClientId`, `testConnection`, `setAuthCodes`, `setSettings`, `sendXmbCommand`, `routeInputToPlayer`, `routeInputToClient`, `saveDataDeepLink`, `rawDataDeepLink`, `invitationDeepLink`, `gameAlertDeepLink`, `systemStatusDeepLink`. Newer bundle evidence also narrows the likely payload shapes: PC `requestClientId()` appears to take no params and emit `gkClientId`, `ps3GKClientID`, and `streamServerClientId`; PC `setSettings()` appears object-shaped; PC auth-code staging may be either a direct `setAuthCodes` command or a second `setSettings` call carrying `gkCloudAuthCode`, `gkPs3AuthCode`, and `streamServerAuthCode`; PC `requestGame()` may reduce to a boolean `forceLogout` call after setup; and a post-`requestGame` `GotLaunchSpec`-style event appears sufficient to push the PC bundle into `startGame`, though real-runtime confirmation is still required. The current saved screenshot still shows only the Plus shell/splash, so real media frames remain beyond the browser-visible boundary for now.
