...
2. **Broker replay / capture (now highest value)** — `broker send <command> [payload]` is now implemented in `psn-direct-cli.ts`, a local `broker:emulator` shim now exists for app-free broker-client iteration on `ws://localhost:1235/`, and a macOS `broker:shell-harness` can now run Sony's real public bundle against that local broker to capture the full BrowserAPI-side sequence (`testConnection -> setSettings -> requestClientId -> setTitleInfo -> setAuthCodes -> requestGame`). The next task is to keep using the mock/harness for outbound-envelope iteration while using a live running app for capturing real replies to the PC-native `requestClientId`, `setSettings`, `setAuthCodes`, `requestGame`, and `startGame` path. Known commands: `startGame`, `stop`, `requestGame`, `requestClientId`, `testConnection`, `setAuthCodes`, `setSettings`, `sendXmbCommand`, `routeInputToPlayer`, `routeInputToClient`, `saveDataDeepLink`, `rawDataDeepLink`, `invitationDeepLink`, `gameAlertDeepLink`, `systemStatusDeepLink`. Newer bundle evidence also narrows the likely payload shapes: PC `requestClientId()` appears to take no params and emit `gkClientId`, `ps3GKClientID`, and `streamServerClientId`; PC `setSettings()` appears object-shaped; PC auth-code staging may be either a direct `setAuthCodes` command or a second `setSettings` call carrying `gkCloudAuthCode`, `gkPs3AuthCode`, and `streamServerAuthCode`; and PC `requestGame()` may reduce to a boolean `forceLogout` call after setup. The BrowserAPI harness additionally confirms directly observed `setTitleInfo`, two-code `setAuthCodes`, and `requestGame` broker envelopes on macOS without the official app.
