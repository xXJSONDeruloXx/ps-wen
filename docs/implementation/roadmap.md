...
2. **Broker replay / capture (now highest value)** — `broker send <command> [payload]` is now implemented in `psn-direct-cli.ts`, and a local `broker:emulator` shim now exists for app-free broker-client iteration on `ws://localhost:1235/`, so the next task is to use the mock for outbound-envelope iteration and a live running app for capturing real replies to `requestClientId`, `setSettings`, `setAuthCodes`, `requestGame`, and `startGame`. Known commands: `startGame`, `stop`, `requestGame`, `requestClientId`, `testConnection`, `setAuthCodes`, `setSettings`, `sendXmbCommand`, `routeInputToPlayer`, `routeInputToClient`, `saveDataDeepLink`, `rawDataDeepLink`, `invitationDeepLink`, `gameAlertDeepLink`, `systemStatusDeepLink`. Newer bundle evidence also narrows the likely payload shapes: PC `requestClientId()` appears to take no params and emit `gkClientId`, `ps3GKClientID`, and `streamServerClientId`; PC `setSettings()` appears object-shaped; PC auth-code staging may be either a direct `setAuthCodes` command or a second `setSettings` call carrying `gkCloudAuthCode`, `gkPs3AuthCode`, and `streamServerAuthCode`; and PC `requestGame()` may reduce to a boolean `forceLogout` call after setup.
